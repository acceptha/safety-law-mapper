...
- **키워드 공유 검증** — `slm validate`가 한 키워드를 3개 이상 매핑이 공유하면 경고(오류 아님). 정당한 공유는 `data/keyword_policy.yaml`에 근거와 함께 등록하면 제외. `ValidationReport.warnings` 추가

### Fixed
- **일반어 `고소작업`·`해체` 소유권 정리** — `고소작업`은 `work-at-height`, `해체`는 `demolition-work`가 소유. 이 둘은 알파벳 tie-break가 아니라 **조건 특이도**가 좁은 매핑을 위로 올려 발생했다(`slm search 해체` → 타워크레인 설치·해체가 1위). 일반어를 빼도 작업명 부분일치로 검색 결과에는 계속 포함되므로 재현율 손실 없음
