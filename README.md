...
flowchart LR
    subgraph data["📂 오픈 데이터 (CC BY 4.0)"]
        ...
        maps["data/mappings/*.yaml<br/>작업→조항 매핑 21건+"]
    end
...
고위험 작업 유형 **21종** 매핑 (10개 카테고리 전부 커버): 밀폐공간, 용접·용단, 밀폐공간×용접(조합), 고소작업, 지게차, 크레인, 굴착, 전기, 비계, 사다리, 중량물, 컨베이어, 프레스, MSDS 화학물질, 위험물질(2개 법령 교차 인용), 해체, 고소작업대, 리프트, 가스용접 등 — 전 조항 [국가법령정보센터](https://www.law.go.kr) 원문 대조 검증 완료.
...
| 단계 | 내용 | 상태 |
|---|---|---|
| 1단계 | 매핑 데이터 + CLI/라이브러리 + 검증 파이프라인 + PyPI 배포 | ✅ v0.1.0 |
